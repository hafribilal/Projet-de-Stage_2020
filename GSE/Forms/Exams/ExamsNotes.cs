...
{
    public partial class ExamsNotes : Form
    {
        ...
        private void ExamsNotes_Load(object sender, EventArgs e)
        {
            Sync();
        ...
        private void combo_Classe_SelectedIndexChanged_1(object sender, EventArgs e)
        {
            dgv.Columns.Clear();
            ...
            foreach (DataGridViewRow row in dgv.Rows)
            {
                ...
                for (int i = 4; i < dgv.Columns.Count; i++)
                {
                    try
                    {
                        ...
                        row.Cells[i].Value = entities.Note_Details.FirstOrDefault(x => x.annee == Program.Annee && x.eleve == id && x.semestere == 0 && x.matiere == matiere).note_Exame;
                    }
                    catch (Exception)
                    {
                ...
                try
                {
                    row.Cells[dgv.Columns.Count - 1].Value = entities.Notes.FirstOrDefault(x => x.eleve == id && x.annee == Program.Annee && x.semester == 0).note_Exame;
                }
                catch (Exception)
                {
