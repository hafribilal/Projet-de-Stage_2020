...
namespace Massar
{
    internal class Tuteurs
    {
        ...
        public Annee_Scolaire GetAnnee(DataTable workSheet)
        {
            ...
            string[] annee = workSheet.Rows[3][6].ToString().Split('/');
            var debut = "03/09/" + workSheet.Rows[3][6].ToString().Substring(0, 4);
            var date_Debut = DateTime.Parse(debut, new System.Globalization.CultureInfo("pt-BR"));
            var fin = "30/06/" + workSheet.Rows[3][6].ToString().Substring(5, 4);
            var date_Fin = DateTime.Parse(fin, new System.Globalization.CultureInfo("pt-BR"));
            return new Annee_Scolaire { annee_Scolaire1 = workSheet.Rows[3][6].ToString(), date_Debut = date_Debut, date_Fin = date_Fin };
        }
