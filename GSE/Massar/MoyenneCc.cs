...
namespace Massar
{
    internal class MoyenneCc
    {
        ...
        public MoyenneCc(DataSet workBook)
        {
        ...
        public List<Eleve> GetEleves(DataTable workSheet)
        {
            ...
            for (int i = 0; i < info.Rows.Count; i++)
            {
                ...
                try
                {
                    note.note1 = Double.Parse(row[row.Table.Columns.Count - 1].ToString());
                }
                catch (Exception x)
                {
                ...
                for (int j = 0; j < not.Columns.Count-1; j++)
                {
                    ...
                    _Details.eleve = eleve.id;
                    _Details.matiere = matieres[j].nom_1;
                    try
                    {
                        _Details.note = double.Parse(row[j].ToString().Replace(',', '.'));
                    }
                    catch (Exception x)
                    {
                        //System.Windows.Forms.MessageBox.Show(row[j].ToString().Replace(',', '.') + "("+j+")\n"+x.Message);
                    }
        ...
        public List<Eleve> GetEleves(DataSet workBook)
        {
            ...
            //System.Windows.Forms.MessageBox.Show(workBook.Tables[0].Rows[3][0].ToString());
            semestre.libelle = workBook.Tables[0].Rows[3][0].ToString();
